...
describe('pipeReducers', () => {
    ...
    it('should pipe reducers result', () => {
        ...
        const piped = pipeReducers(spoiledReducer1, reducer, spoiledReducer2);
        piped('initialState', 'action');

        expect(spoiledReducer1.calls.allArgs())
            .toEqual([['initialState', 'action']]);
        expect(reducer.calls.allArgs())
            .toEqual([['spoiledResult1', 'action']]);
        expect(spoiledReducer2.calls.allArgs())
            .toEqual([['pureResult', 'action']]);
    });
    ...
    it('should use original effect when sinlge reducer spoiled', () => {
        ...
        expect(pipedReducer('state', 'action'))
            .toEqual(spoiled('state', 'effect'));
